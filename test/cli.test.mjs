...
test("successful help, version, option boundaries, and parser-error exit handling", () => {
  ...
  assert.equal(version.stdout, "messcript 0.1.4\n");
...
test("JSON reports include metadata, findings, and processing errors", () => {
  ...
  assert.deepEqual(report.tool, { name: "messcript", version: "0.1.4" });
...
test("XML, Checkstyle, SARIF, and report files preserve the report contract", () => {
  ...
  assert.match(xml.stdout, /<messcript version="0\.1\.4">/);
  ...
  assert.match(checkstyle.stdout, /^<checkstyle tool="messcript" version="0\.1\.4">/);
